...
import time
import cv2 as cv
from vision import Vision, MobInfoFilter
import numpy as np
import enum
...
class MetinFarmBot:
    ...
    def __init__(self, metin_window, metin_selection):
        ...
        self.osk_window.move_window(x=-0, y=0)
            ...
            if self.state == BotState.SEARCHING:
                ...
                if self.screenshot is not None and self.detection_time is not None and \
                    ...
                    if self.detection_result is None:
                        ...
                        self.detection_result['click_pos'][0] = int(self.detection_result['click_pos'][0] * 1.25 + 60)
                        self.detection_result['click_pos'][1] = int(self.detection_result['click_pos'][1] * 1.25 + 60)
    ...
    def turn_on_buffs(self):
        ...
        self.osk_window.press_key(button='3', mode='click')
        self.osk_window.press_key(button='3', mode='click')
        time.sleep(1.5)
        self.osk_window.un_mount()
    ...
    def activate_elixir(self):
        self.osk_window.press_key(button='1', mode='click')
        time.sleep(0.1)
        self.osk_window.press_key(button='2', mode='click')
        time.sleep(0.2)
